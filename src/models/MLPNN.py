...
class MLPNN():
	...
	def __init__(self, input_len, num_class, config):
		self.num_fc_nodes = int(config.get('Benchmark', 'NumFCNodes_MLPNN'))
		self.num_fc_layers = int(config.get('Benchmark', 'NumFCLayers_MLPNN'))
		self.lamb = float(config.get('Benchmark', 'L2_Lambda_MLPNN'))
		self.drop = float(config.get('Benchmark', 'Dropout_MLPNN'))
