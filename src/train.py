...
def train_PopPhy(txtName,model_name,addWeight_Method,config):
	...
	dadhmaps, my_maps, _, _, _, tree_features, labels, label_set, g, feature_df = prepare_Data(path, config,addWeight_Method)
	...
	if model_name=='topo-gcn':   #topo-cnn\topo-gcn
		data = np.expand_dims(dadhmaps,-1)
	else:
		data = my_maps
	...
	stat_df = pd.DataFrame(index=["AUC", "Accuracy","F1","Precision", "Recall","MCC"], columns=cv_list)
	stat_result ={}
	#########################################################################
	...
	for seed in seeds:
		...
		# num_test ='NumberTestSplits'=几个交叉
		fold = 0
		...
		for train_index, test_index in skf.split(data, labels):
			...
			popphy_model = PopPhyCNN((tree_row, tree_col), num_class,model_name,config)
			# popphy_model.model.build()
			if fold + run == 0:
				print(popphy_model.model.summary())	
				print("\n\n Run\tFold\t%s" % (metric))

			popphy_model.train(train, train_weights)					
			preds, stats, param = popphy_model.test(test)
			if num_class == 2:
				stat_df.loc["AUC"]["Run_" + str(run) + "_CV_" + str(fold)]=stats["AUC"]
			stat_df.loc["MCC"]["Run_" + str(run) + "_CV_" + str(fold)]=stats["MCC"]
			stat_df.loc["Accuracy"]["Run_" + str(run) + "_CV_" + str(fold)] = stats["Accuracy"]
	...
	txtresult.write(str(param) + "\n")
	txtresult.write('time_use = '+time_use + "\n")
	txtresult.write(str(result)+'\n')
	txtresult.write("--------------------------------------------\n")
	...
	# stat_df.mean(1).to_csv(txtName,mode='a+')
	# txtresult = open ( txtName , "a+" )
	# txtresult.write ( "k=" + str(k) + "\nm=" + str(m)+"\n" +"b=10\n")
	# txtresult.write(param + "\n")
	# txtresult.write("--------------------------------------------\n")
...
if __name__ == "__main__":
	...
	# 看好m、k、b
	config = get_config()
	model_name = 'topo-cnn'  # topo-cnn\topo-gcn
	addWeight_Method='Patri'   # Height\Patri\Children
	dataset = config.get('Evaluation', 'DataSet')
	ticks = time.strftime("%m%d_%H%M", time.localtime())
	txtName = str(dataset) + str(ticks) + '_' + model_name +'_'+addWeight_Method+'_result.csv'
		...
		for b in range(5, 15, 5):
			...
			for m in range(3, 9, 2):
				...
				for k in np.arange(Decimal('-1.0'), Decimal('-0.0'), Decimal('0.1')):
					...
					train_PopPhy(txtName,model_name,addWeight_Method,config)
	elif addWeight_Method == 'Children':
		for m in range(3, 9, 2):
			config.set('GCN', 'm', str(m))
			for k in np.arange(Decimal('0.2'), Decimal('2.0'), Decimal('0.1')):
				config.set('GCN', 'k', str(k))
				train_PopPhy(txtName,model_name,addWeight_Method,config)
	elif addWeight_Method == 'Patri':
		for m in range(3, 9, 2):
			config.set('GCN', 'm', str(m))
			for k in np.arange(Decimal('0.2'), Decimal('3.2'), Decimal('0.2')):
				config.set('GCN', 'k', str(k))
				train_PopPhy(txtName,model_name,addWeight_Method,config)
	else:
		train_PopPhy(txtName, model_name, addWeight_Method, config)
