...
class Graph:
	...
	def routeToRoot( self,config):
		layers = self.layers  # 等于实际层数,但g.get_nodes(i)的i是从1开始记录，第0层为空
		width = self.width  # 树的最大宽度
		path='../data/'+config.get('Evaluation', 'DataSet')
		self.write_table ( path+"/newick_table.txt" )  # 存储所有节点与孩子的对应关系表
		# 1、按层存储每层的所有节点，即最后特征在矩阵对应的位置
		# 2、routeForAll记录所有节点(包括中间节点和叶子节点、根)到根的路径
		# 3、leaves记录所有叶子节点
		file = open (path+"/layer_node.txt" , "w" )
		routeForAll = {}
		leaves = [ ]
		...
		routeFile = open ( path+"/route_toRoot.txt" , "w" )
	...
	def addWeight_Height( self , k , m ,b):
	...
	def get_Hmatrix(self):
		allnodes = self.my_get_all_nodes()
		outmetrix = np.zeros(len(allnodes))
		for node in allnodes:
			index = allnodes.index(node)
			node_value = node.get_abundance()
			outmetrix[index] = node_value
		return outmetrix
	...
	def getgcninfo(self):
		allnodes = self.my_get_all_nodes()
		#树中结点ab两两遍历
		adjmetrix = np.zeros((len(allnodes),len(allnodes)))
		degreemetrix = np.zeros((len(allnodes),len(allnodes)))
		for node_a in allnodes:
			index_a = allnodes.index(node_a)
			for node_b in allnodes:
				index_b = allnodes.index(node_b)
				linka = []
				linka = linka + node_a.get_children()
				linka.append(node_a.get_parent())
				linka = set(linka)

				degreemetrix[index_a,index_a] = len(linka)

				if node_b in linka:
					# print("relation:"+str(node_a.id)+" and "+str(node_b.id))
					adjmetrix[index_a, index_b] = 1
					adjmetrix[index_b, index_a] = 1

		normaladj = normalize_adj(adjmetrix)
