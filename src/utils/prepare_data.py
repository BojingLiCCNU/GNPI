...
def generate_maps(x,g,f,config,addWeight_Method,normaladj):
	...
	if addWeight_Method=='Children':
		...
		temp_g.addWeight_Height(k,m,b)
	elif addWeight_Method=='Patri':
		temp_g.addWeight_Patri(k, m)
	else:  # 此处均不加权
		pass
	map = temp_g.get_map()   # （加权/不加权）树形矩阵
	hmatrix = temp_g.get_Hmatrix()  # （加权/不加权）H向量
	dadhmaps = np.matmul(normaladj,hmatrix)  # （加权/不加权）gcn中的DAD结果
	#----------------------------------------------------
...
def prepare_Data(path, config,addWeight_Method):
	...
	results = Parallel(n_jobs=num_cores)(delayed(generate_maps)(x, g, features_df,config,addWeight_Method,normaladj) for x in data.values)
   ...
   #如果 results 是一个numpy array，results.take(m,1)表示取每一行的第m个值；results.take(m,0)表示取第m行
	# results 每一行为一个 x 经 generate_maps 处理的结果[x, np.array(map), np.array(vector), hmatrix, dadhmaps]
	my_benchmark = np.array(np.take(results, 0, 1).tolist())  # 每一个x组成的矩阵，未做任何填充得到的矩阵(原丰度矩阵,用于ML)
	my_maps = np.array(np.take(results,1,1).tolist())  # 每一个x（加权/不加权）后的树形填充得到的矩阵(用于topopcnn)
	dadhmaps = np.array(np.take(results,4,1).tolist())  # 每一个x（加权/不加权）经gcn得到的矩阵（用于gcn-topocnn/ML）
